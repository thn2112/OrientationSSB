...
res_dir = res_dir + 'sbi_l23_mod_patt_broad_narrow/'
if not os.path.exists(res_dir):
    os.makedirs(res_dir)
...
if bayes_iter == 0:
    prior = BoxUniform(low =torch.tensor([ 0.0,-2.0,-3.0,-2.0, 0.0075, 0.02, 0.1, 0.1, 0.01, 0.01],device=device),
                       high=torch.tensor([ 1.0, 2.0,-0.0, 1.0, 0.02  , 0.1 , 1.5, 1.5, 0.5 , 0.10],device=device),)

    prior,_,_ = process_prior(prior)
else:
    with open(f'./../notebooks/l23_patt_broad_narrow_posterior_{bayes_iter:d}.pkl','rb') as handle:
...
def get_sheet_resps(theta,N):
    ...
    for prm_idx in range(theta.shape[0]):
        kern_nar = np.exp(-(dss/(theta[prm_idx,4].item()))**2)
        norm = kern_nar.sum(axis=1).mean(axis=0)
        kern_nar /= norm
        
        kern_bro = np.exp(-(dss/(theta[prm_idx,5].item()))**2)
...
def sheet_simulator(theta):
    ...
    theta[:,4] = s_n
    theta[:,5] = s_b
    theta[:,6] = Je_narrow / Je_broad
    theta[:,7] = Ji_narrow / Ji_broad
    theta[:,8] = het_level
    theta[:,9] = inp_str

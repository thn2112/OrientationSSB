...
e_opm_fft = rng.normal(size=(N,N)) + 1j * rng.normal(size=(N,N))
i_opm_fft = rng.normal(size=(N,N)) + 1j * rng.normal(size=(N,N))
s_opm_fft = (e_opm_fft + i_opm_fft) / np.sqrt(2)
d_opm_fft = (e_opm_fft - i_opm_fft) / np.sqrt(2)
s_opm_fft[0,0] = 0
d_opm_fft[0,0] = 0
del e_opm_fft, i_opm_fft

freqs = np.fft.fftfreq(N,1/N)
freqs = np.sqrt(freqs[:,None]**2 + freqs[None,:]**2)

decay = 5
s_opm_fft *= (1+EI_corr)*np.exp(-freqs/decay)
d_opm_fft *= (1-EI_corr)*np.exp(-freqs/decay)

s_opm = np.fft.ifft2(s_opm_fft)
d_opm = np.fft.ifft2(d_opm_fft)
e_opm = (s_opm + d_opm) / np.sqrt(2)
i_opm = (s_opm - d_opm) / np.sqrt(2)
del s_opm_fft, d_opm_fft, s_opm, d_opm

for opm in [e_opm, i_opm]:
...
opms = np.concatenate((e_opm.flatten(),i_opm.flatten()))
pos = np.angle(opms)
oss = np.abs(opms)
...
def integrate_ring(xea0,xen0,xeg0,xia0,xin0,xig0,inp,Jee,Jei,Jie,Jii,ne,ni,threshe,threshi,
                   ...
                   frac_ee_broad=0.5,frac_ei_broad=0.5,frac_ie_broad=0.5,frac_ii_broad=0.5):
    '''
    ...
    if np.isscalar(Jee):
        ...
        w_full[0,:,0,:] = Jee * (kernn + frac_ee_broad * kernb)
        w_full[0,:,1,:] = Jei * (kernn + frac_ei_broad * kernb)
        w_full[1,:,0,:] = Jie * (kernn + frac_ie_broad * kernb)
        w_full[1,:,1,:] = Jii * (kernn + frac_ii_broad * kernb)
...
def get_resps(theta):
    ...
    oris = (np.linspace(0,np.pi,nori,endpoint=False)[None,:,None] * np.ones((2*nos,1,nphs))).flatten()
    phss = (np.linspace(0,2*np.pi,nphs,endpoint=False)[None,None,:] * np.ones((2*nos,nori,1))).flatten()
    
    resps = np.zeros((theta.shape[0],2,nos,nori,nphs))
    for prm_idx in range(theta.shape[0]):
        oss = np.array([0.1,0.3,0.5,0.7])
        oss = np.concatenate((oss,theta[prm_idx,10].item()*oss))
        gams = (gam_os_itp(oss)[:,None,None] * np.ones((1,nori,nphs))).flatten()
        amp_mult = np.concatenate((np.ones(nperpop),theta[prm_idx,11].item()*np.ones(nperpop)))
        def ff_inp(t):
            return c*elong_inp(gams,oris,phss+2*np.pi*3*t,amp_mult=amp_mult)

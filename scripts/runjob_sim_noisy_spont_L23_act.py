            ...
            for sf in sfs:
                for inp_str in iss:
                    for seed in seeds:
                        #--------------------------------------------------------------------------
                        # Make SBTACH
                        inpath = currwd + "/sim_noisy_spont_L23_act.py"
                        c1 = "{:s} -s {:d} -npt {:d} -pcv {:.2f} -sf {:d} -is {:.1f} -r 1".format(
                                inpath,seed,n_patt,pcv,sf,inp_str)
                        res_dir = './../results/L23_sel/'
                        if os.path.isfile(res_dir+'spont_noisy_cv={:.2f}_spat_freq={:d}_inp_str={:.1f}_seed={:d}.pkl'.format(pcv,sf,inp_str,seed)):
                            continue

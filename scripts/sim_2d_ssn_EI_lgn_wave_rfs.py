...
from kayser_model_2d_ssn_EI import Model
...
if test:
    res_dir = res_dir + 'sim_2d_ssn_EI_lgn_{:s}_rfs_ne={:d}_ni={:d}/'.format(mode,n_e,n_i)
else:
    res_dir = res_dir + 'sim_2d_ssn_EI_lgn_{:s}_rfs_ng={:d}_ne={:d}_ni={:d}_sx={:.2f}_ss={:.2f}_gi={:.1f}_w={:.1f}_p={:d}_r={:d}_d={:.1f}/'.format(
...
if init_iter+batch_iter < max_iter:
    os.system("python runjob_sim_2d_ssn_EI_lgn_wave_rfs.py " + \

...
parser.add_argument('--w_arbor', '-w', help='width of L4 to L2/3 arbor in pixels',type=int, default=4)
...
for inp_seed_idx in range(num_inp_seeds):    
    try:
        with open(l4_dir + 'seed={:d}.pkl'.format(inp_seed_idx),'rb') as handle:
    ...
    for rec_seed_idx in range(num_rec_seeds):
        try:
            with open(res_dir + 'arbored_w_arb={:d}_inp_seed={:d}_rec_seed={:d}.pkl'.format(w_arbor,inp_seed_idx,rec_seed_idx),'rb') as handle:
        ...
        inp_os_samp = np.abs(inp_opm).flatten()[samp_idxs[inp_seed_idx]]
        rate_os_samp = np.abs(rate_opm).flatten()[samp_idxs[inp_seed_idx]]
        inp_po_samp = inp_po.flatten()[samp_idxs[inp_seed_idx]]
        rate_po_samp = rate_po.flatten()[samp_idxs[inp_seed_idx]]
        inp_mr_samp = inp_mr.flatten()[samp_idxs[inp_seed_idx]]
        rate_mr_samp = rate_mr.flatten()[samp_idxs[inp_seed_idx]]
        ...
        inp_os_samps[inp_seed_idx,rec_seed_idx] = inp_os_samp
        inp_po_samps[inp_seed_idx,rec_seed_idx] = inp_po_samp
        inp_mr_samps[inp_seed_idx,rec_seed_idx] = inp_mr_samp
        inp_fpss[inp_seed_idx,rec_seed_idx] = inp_fps
        inp_corr_curves[inp_seed_idx,rec_seed_idx] = inp_corr_curve
        rate_os_samps[inp_seed_idx,rec_seed_idx] = rate_os_samp
        rate_po_samps[inp_seed_idx,rec_seed_idx] = rate_po_samp
        rate_mr_samps[inp_seed_idx,rec_seed_idx] = rate_mr_samp
        rate_fpss[inp_seed_idx,rec_seed_idx] = rate_fps
        rate_corr_curves[inp_seed_idx,rec_seed_idx] = rate_corr_curve
        mismatch_samps[inp_seed_idx,rec_seed_idx] = mismatch_samp
        mods[inp_seed_idx,rec_seed_idx] = mod
        dims[inp_seed_idx,rec_seed_idx] = dim
...
with open(res_dir + 'arbored_analysis.pkl', 'wb') as handle:

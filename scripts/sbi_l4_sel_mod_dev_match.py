...
res_dir = res_dir + 'sbi_l4_sel_mod_dev_match/'
if not os.path.exists(res_dir):
    os.makedirs(res_dir)
...
opm_fft *= np.exp(-freqs/2) + 0.5*np.exp(-freqs/50)

omap = np.fft.ifft2(opm_fft)
omap *= np.abs(omap)**1.6/np.abs(omap)
omap *= 0.12 / np.median(np.abs(omap)) # normalize median to data
    ...
    for prm_idx in range(theta.shape[0]):
        ...
        for ori_idx,ori in enumerate(oris):
            ...
            resp = integrate_sheet(np.zeros(N**2),np.zeros(N**2),np.zeros(N**2),
                                    ...
                                    thresh,thresh,0,dt,nwrm+nphs,tsamp,
